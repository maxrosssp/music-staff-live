import cv2
import numpy as np
from msl.sight.line import *
from pydub import AudioSegment
import random
import sys
import os
from msl.sound import *

class RawStaff:

	def __init__(self, image, **kwargs):
		# CANNY_THRESHOLD1=50, CANNY_THRESHOLD2=150, CANNY_APERTURESIZE=3, RHO=1, THETA=180, THRESHOLD=200):
		self.CANNY_THRESHOLD1 = kwargs.get('CANNY_THRESHOLD1', 50)
		self.CANNY_THRESHOLD2 = kwargs.get('CANNY_THRESHOLD2', 150)
		self.CANNY_APERTURESIZE = kwargs.get('CANNY_APERTURESIZE', 3)
		self.RHO = kwargs.get('RHO', 1)
		self.THETA = kwargs.get('THETA', 180)
		self.THRESHOLD = kwargs.get('THRESHOLD', 200)
	...
	def group_hori_line_heights(self, lines):
		line_height_groups = [[],[]]
		if lines != []:
			for line in lines:
				line_height = line.midpoint_height()
				if line_height <= (.223 * self.image_height):
					line_height_groups[0].append(line_height)
				elif line_height >= (.636 * self.image_height):
					line_height_groups[1].append(line_height)
		return line_height_groups

	def extract_board(self):
		all_lines = cv2.HoughLines(self.edges,self.RHO,np.pi/self.THETA, self.THRESHOLD)
		hori_lines, _ = self.filter_orient_size_and_outliers(all_lines)

		hori_line_height_groups = self.group_hori_line_heights(hori_lines)
		if hori_line_height_groups[0] != [] and hori_line_height_groups[1] != []:
			board_top = max(hori_line_height_groups[0]) 
			board_bottom = min(hori_line_height_groups[1])

			image = self.image[(board_top + 15):(board_bottom - 15), 0:self.image_width]
			return image
		return self.image
...
class Staff:
	...
	def find_staff_lines(self):
		all_staff_lines = cv2.HoughLines(self.edges,self.RHO,np.pi/self.THETA, self.THRESHOLD)
	...
	def find_vert_lines(self):
		all_vert_lines = cv2.HoughLines(self.edges,self.RHO,np.pi/self.THETA, self.THRESHOLD)
	...
	def extract_staff(self):
		...
		# 	print line ,
		# print ""
		# print "Staff lines: " + str(self.staff_lines)
		...
		# print "Btop: " + str(btop)
		# print "Vertical lines: " + str(self.vertical_lines)
...
class Staffy:
	...
	def __init__(self, image, m1_start, m2_start, **kwargs):
		# CANNY_THRESHOLD1=50, CANNY_THRESHOLD2=150, CANNY_APERTURESIZE=3, RHO=1, THETA=180, THRESHOLD=200,
		...
		self.THETA = kwargs.get('THETA', 180)
		self.THRESHOLD = kwargs.get('THRESHOLD', 200)
		self.DP = kwargs.get('DP', 1)
		self.MINDIST = kwargs.get('MINDIST', 33)
		self.PARAM1 = kwargs.get('PARAM1', 51)
		self.PARAM2 = kwargs.get('PARAM2', 27)
		self.MINRADIUS = kwargs.get('MINRADIUS', 9)
		self.MAXRADIUS = kwargs.get('MAXRADIUS', 25)
		# self.BLUE_COLOR_PARAMS = kwargs.get('BLUE_COLOR_PARAMS', {'b1': 101,'g1': 45,'r1': 30,
  #       															'b2': 180,'g2': 143,'r2': 101})
		# self.PINK_COLOR_PARAMS = kwargs.get('PINK_COLOR_PARAMS', {'b1': 0,'g1': 0,'r1': 75,
  #       															'b2': 195,'g2': 79,'r2': 206})
		self.BLUE_B1 = kwargs.get('BLUE_B1',101)
		self.BLUE_G1 = kwargs.get('BLUE_G1',45)
		self.BLUE_R1 = kwargs.get('BLUE_R1',30)
		self.BLUE_B2 = kwargs.get('BLUE_B2',180)
		self.BLUE_G2 = kwargs.get('BLUE_G2',143)
		self.BLUE_R2 = kwargs.get('BLUE_R2',101)
		self.PINK_B1 = kwargs.get('PINK_B1',0)
		self.PINK_G1 = kwargs.get('PINK_G1',0)
		self.PINK_R1 = kwargs.get('PINK_R1',75)
		self.PINK_B2 = kwargs.get('PINK_B2',195)
		self.PINK_G2 = kwargs.get('PINK_G2',79)
		self.PINK_R2 = kwargs.get('PINK_R2',206)
  ...
  #                           param1=50,param2=30,minRadius=0,maxRadius=int(self.image_width/40.0))
		circles_list = cv2.HoughCircles(gray,cv2.HOUGH_GRADIENT,self.DP,self.MINDIST,
                            param1=self.PARAM1,param2=self.PARAM2,minRadius=self.MINRADIUS,maxRadius=self.MAXRADIUS)
		self.circles_list = np.uint16(np.around(circles_list))

		self.circles = []
		centers = []
		for circles in self.circles_list:
			# print "1. Num of circles before: " + str(len(circles))
			for circle in circles:
				# print "2. " + str(circles)
				# print "3. " + str(circle[0]) + ", " + str(circle[1])

				center_pixel = self.image[circle[1],circle[0]+9]
				if self.pixel_in_range(center_pixel):
					self.circles.append(circle)
					centers.append((circle[0],circle[1]))
					# circles = np.delete(circles, circle,0)
				# else:
				# 	self.circles.append(circle)
				# 	centers.append((circle[0],circle[1]))

		# 	print "Num of circles after: " + str(len(circles))
		# print "NEXT"
		# print ""
		self.circle_centers = centers

		# print "Circles type: " + str(type(circles))
		# self.circles = np.uint16(np.around(circles_list))
		# print "Circles type after uint16: " + str(type(self.circles))
		
		# centers = []
		# print "Number of circles before: " + str(len(self.circles[0]))
		# for i in self.circles[0,:]:
		# print i
		# draw the center of the circle
		# center_pixel = self.image[i[1],i[0]]
		# if self.pixel_in_range(center_pixel):
			# centers.append((i[0],i[1]))
		# else:
			# np.delete(self.circles[0], i)
	# print "Number of circles after: " + str(len(self.circles[0]))

		# self.circle_centers = centers

	def pixel_in_range(self, pixel):
		in_range = False
		if (self.BLUE_B1 <= pixel[0] <= self.BLUE_B2) and (self.BLUE_G1 <= pixel[1] <= self.BLUE_G2) and (self.BLUE_R1 <= pixel[2] <= self.BLUE_R2):
			in_range = True
		if (self.PINK_B1 <= pixel[0] <= self.PINK_B2) and (self.PINK_G1 <= pixel[1] <= self.PINK_G2) and (self.PINK_R1 <= pixel[2] <= self.PINK_R2):
			in_range = True
		return in_range
...
class Staffer:
	...
	def __init__(self, image, tempo=120, **kwargs):
		# CANNY_THRESHOLD1=50, CANNY_THRESHOLD2=150, CANNY_APERTURESIZE=3, RHO=1, THETA=180, THRESHOLD=200,
		# DP=1, MINDIST=33, PARAM1=51, PARAM2=27, MINRADIUS=9, MAXRADIUS=25):
		self.image = image
		self.raw_staff = RawStaff(self.image, **kwargs)
		self.staff = Staff(self.raw_staff.extract_board(), **kwargs)
		self.staffy = Staffy(self.staff.extract_staff(), self.staff.get_m1_start(), self.staff.get_m2_start(), **kwargs)
